...
def get_recommendations():
    ...
    if request.method == 'POST':
        criteria = {
            ...
            'min_renewable': float(request.form.get('min_renewable', 0)) 
                              if request.form.get('min_renewable') else None,
            'sustainability': request.form.get('sustainability')
        }
    
    recommendations = recommender.get_supplier_recommendations(criteria)
    
    # If no recommendations found, get default recommendations
    if not recommendations:
        recommendations = recommender.get_default_recommendations()
        
    # Add a message to the template context if we had to use default recommendations
    show_alternative_message = len(recommendations) > 0 and criteria is not None
    
    return render_template('recommendations1.html', 
                          recommendations=recommendations, 
                          show_alternative_message=show_alternative_message)
...
def supplier_details(supplier_id):
    db = Database()
    details = db.get_supplier_details(supplier_id)
    db.close()
    if details:
        return render_template('supplier_details.html', supplier=details)
...
# Create directories for templates if they don't exist
os.makedirs('templates', exist_ok=True)

if __name__ == '__main__':
    # Train or load the model when the app starts
